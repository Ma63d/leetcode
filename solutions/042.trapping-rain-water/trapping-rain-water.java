public class Solution {
    ...
    public int trap(int[] height) {
        ...
        int lastLeft = height[left], lastRight = height[right];
        int size = 0;
        while(left < right){
            if(height[left] < height[right]){
                if(height[left+1] < lastLeft){
                    size += lastLeft - height[left+1];
                }else if(height[left+1] > lastLeft){
                    lastLeft = height[left+1]; 
                }
                left++;
            }else{
                if(height[right-1] < lastRight){
                    size+= lastRight - height[right-1];
                }else if(height[right-1] > lastRight){
                    lastRight = height[right-1];
                }
                right--;
            }
        }
